...
const {
  ...
  updateTask,
  deleteTask,
  createAssignedTask, // NUEVO
  updateAssignedTask, // NUEVO
  deleteAssignedTask, // NUEVO
  createCategory,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {isAdmin ? (
          <AdminPanel 
            ...
            assignedTasks={assignedTasks} // NUEVO
            ...
            createAssignedTask={createAssignedTask} // NUEVO
            updateAssignedTask={updateAssignedTask} // NUEVO
            deleteAssignedTask={deleteAssignedTask} // NUEVO
          />
        ) : (
          <EmployeePanel 
            user={currentUser} 
...
export default App;  // Componente Seguimiento
  const TrackingPanel = () => {
...
const AdminPanel = memo(({ 
  ...
  // NUEVO: Componente de Gestión de Tareas Asignadas
  const AssignedTasksManagement = () => {
    ...
    const [filters, setFilters] = useState({
      empleado: '',
      status: 'todas', // todas, pendiente, en_progreso, completada
      priority: 'todas' // todas, alta, media, baja
    });
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Formulario para nueva tarea asignada */}
        {showNewTaskForm && (
          <div className="bg-white rounded-lg shadow-sm border p-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-semibold mb-4">Filtros</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 import React, { useState, useEffect, useCallback, useMemo, memo } from 'react';
...
const useSupabaseData = () => {
  ...
  const [assignedTasks, setAssignedTasks] = useState([]); // NUEVO: Tareas asignadas
  ...
  const loadAllData = useCallback(async () => {
    try {
      const [usersRes, tasksRes, categoriesRes, justifiedRes, archivedRes, assignedRes] = await Promise.all([
        ...
        supabase.select('assigned_tasks') // NUEVO: Cargar tareas asignadas
      ]);
      ...
      if (assignedRes.data) setAssignedTasks(assignedRes.data); // NUEVO
  ...
  // NUEVO: Funciones CRUD para tareas asignadas
  const createAssignedTask = useCallback(async (taskData) => {
    try {
      console.log('🚀 Creando tarea asignada:', taskData);
      const { data, error } = await supabase.insert('assigned_tasks', taskData);
      if (error) {
        console.error('❌ Error creando tarea asignada:', error);
        throw error;
      }
      console.log('✅ Tarea asignada creada exitosamente:', data);
      await loadAllData();
      return { success: true };
    } catch (err) {
      console.error('❌ Error en createAssignedTask:', err);
      return { success: false, error: err.message };
    }
  }, [loadAllData]);
...
// NUEVO: Componente Tareas Asignadas para Empleados
